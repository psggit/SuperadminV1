...
const Lister = ({limit, currentPage, showMax, count, onClickHandler}) => {
  ...
  */
  ...
  leftPage = (isLeftPage) ? (currentPage - 1) : currentPage;
  rightPage = (isRightPage) ? (currentPage + 1) : currentPage;
  ...
  pageHtml = Array.apply(null, Array(currentShowMax)).map((p, i) => {
    ...
    return (
          <li key={i} className={(isActive) ? styles.active : ''}>
            <Link to={ i } onClick={onClickHandler}>
              {startVal++}
            </Link>
          </li>
        );
