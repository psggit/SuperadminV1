...
class ViewDebitCredit extends React.Component { // eslint-disable-line no-unused-vars
  ...
  render() {
    ...
    const fields = [ 'id', 'bar.name', 'bar_credits_and_debit_codes.code', 'amount', 'comment' ];
    // const operator = ['$eq'];
    const fieldOperatorMap = {
      'bar.name': ['$eq', '$like', '$ilike'],
      'bar_credits_and_debit_codes.code': ['$eq', '$like', '$ilike'],
      'id': ['$eq', '$gt', '$lt'],
      'amount': ['$eq', '$gt', '$lt'],
    ...
    const fieldTypeMap = {
      'bar.name': 'text',
      'bar_credits_and_debit_codes.code': 'text',
      'comment': 'text',
      'id': 'number',
      'amount': 'number'
    };
    ...
    return (
        <div className={styles.container}>
          ...
          <div className={styles.create_layout + ' ' + styles.wd_100}>
            <Link to={'/hadmin/bar_management/debit_credit_transactions'}>
          ...
          <PaginationWrapper
            ...
            showMax="5"
            parentUrl="/hadmin/bar_management/debits_credits_landing"
          />
        </div>
      );
  }
