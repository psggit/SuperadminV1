...
const fetchBrandManager = (dispatch, getState) => {
  const promises = [];
  const company = getState().promosOnPackRedeemState.company;
  const selectBrandManagersQuery = selectBrandManagers(company);
...
const mapStateToProps = (state) => {
  return {...state.promosOnPackRedeemState, ...state.loginState};
};
  ...
  return {
    onSubmitData: (values, e) => {
      ...
      if (validators(campaignValidatorsDict, 'brandEmail', values.brandEmail, values) &&
        ...
        let proceed = confirm('`Onpack Promos Campaign` are currently UnEditable. You currently have (' + values.promos.length + ') promos listed under this campaign.\n Are you sure you want to continue?');
        if (proceed === false) {
          return;
        }
