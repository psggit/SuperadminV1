...
class WelcomeDrinksView extends React.Component { // eslint-disable-line no-unused-vars
  constructor() {
    super();
  ...
  render() {
    ...
    const fields = [ 'id', 'product.sku.brand.brand_name', 'store_type', 'active_to', 'active_from' ];
    // const operator = ['$eq'];
    const fieldOperatorMap = {
      'id': ['$eq', '$gt', '$lt'],
      'product.sku.brand.brand_name': ['$eq', '$like', '$ilike'],
      'store_type': ['$eq', '$like', '$ilike'],
      'active_to': ['$eq', '$lt', '$gt'],
      'active_from': ['$eq', '$lt', '$gt']
    };
    const fieldTypeMap = {
      'id': 'number',
      'product.sku.brand.brand_name': 'text',
      'store_type': 'text',
      'active_to': 'date',
      'active_from': 'date'
    };
    ...
    return (
        <div className={styles.container}>
          ...
          <PaginationWrapper
            ...
            showMax="5"
            parentUrl="/hadmin/brands_offers_and_promos/welcome_drinks_view"
          />
        </div>
      );
  }
...
const decoratedConnectedComponent = commonDecorator(WelcomeDrinksView);// connect(mapStateToProps)(CommonDecorator);
