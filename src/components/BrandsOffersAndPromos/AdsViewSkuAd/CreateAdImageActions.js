...
const SKU_FETCH = 'AD_VIEW_SKU/SKU_FETCH';
const BRAND_MANAGER_FETCH = 'AD_VIEW_SKU/BRAND_MANAGER_FETCH';
...
const AD_FETCH = 'AD_VIEW_SKU/AD_INFO';
...
export const fetchAd = (id) => {
  return (dispatch, getState) => {
    /* Url */
    const url = Endpoints.db + '/table/ad_sku/select';
    const queryObj = {};
    ...
    return Promise.all([
      ...
      const brandManId = getState().viewSkuAd_data.campaignDetails.brand_manager_id;
      ...
      const skuId = getState().viewSkuAd_data.campaignDetails.sku_id;
...
const adsViewSkuReducer = (state = defaultcreateImageAdData, action) => {
  switch (action.type) {
...
export default adsViewSkuReducer;
