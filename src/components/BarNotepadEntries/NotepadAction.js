...
const BAR_NOTEPAD_FETCH_ISSUE_SUCCESS = 'BAR_NOTEPAD/NOTEPAD_FETCH_ISSUE_SUCCESS';
...
const fetchNotepad = (userId) => {
  return (dispatch, getState) => {
    /* Url */
    const url = Endpoints.db + '/table/bar_notepad/select';
    const queryObj = {};
    queryObj.columns = [
      '*',
      {
...
const fetchIssueTypes = () => {
  return (dispatch, getState) => {
    ...
    dispatch({type: MAKE_REQUEST});
    return Promise.all([
      dispatch(requestAction(url, options, BAR_NOTEPAD_FETCH_ISSUE_SUCCESS, REQUEST_ERROR)),
...
const insertNotepad = (issueId, description, userId) => {
  return (dispatch, getState) => {
    const url = Endpoints.db + '/table/bar_notepad/insert';
    ...
    return dispatch(requestAction(url, options))
      .then((resp) => {
        if (resp.returning.length > 0) {
          ...
          dispatch(routeActions.push('/hadmin/bar/profile/' + currentUserId + '/view_notepads'));
        }
      })
      .catch((resp) => {
        console.log(resp);
...
const barNotepadReducer = (state = defaultNotepadState, action) => {
  switch (action.type) {
    case BAR_NOTEPAD_FETCH_ISSUE_SUCCESS:
...
export default barNotepadReducer;
