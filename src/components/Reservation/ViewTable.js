...
class ViewReservation extends Component {
  ...
  render() {
    const {tableName, schemas, query, curFilter, rows,  // eslint-disable-line no-unused-vars
           activePath, ongoingRequest, lastError, lastSuccess, dispatch, consumer_reservation_id} = this.props; // eslint-disable-line no-unused-vars
    ...
    const viewRows = (<ViewRows curTableName={tableName}
                             ...
                             curDepth={0}
                             dispatch={dispatch} />);
    const tableHeader = (<TableHeader title={"Consumer Reservation details"} />);
...
ViewReservation.propTypes = {
  ...
  consumer_reservation_id: PropTypes.string.isRequired
};
