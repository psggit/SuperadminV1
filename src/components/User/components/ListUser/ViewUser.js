...
class ViewUser extends Component {
  ...
  fetchInitialData(page, limit) {
    this.props.dispatch(getAllUserData(page, limit));
  }
  triggerPageChange(clickedPage, limit) {
    this.props.dispatch(getAllUserData(clickedPage, limit));
  }
  enableSearch() {
    const page = 1;
    ...
    Promise.all([
      ...
      this.props.dispatch(getAllUserData(page, 10))
    ])
    .then( () => {
  ...
  render() {
    ...
    const styles = require('./ViewUser.scss');
...
const decoratedConnectedComponent = commonDecorator(ViewUser);// connect(mapStateToProps)(CommonDecorator);
