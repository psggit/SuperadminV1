...
class CreateNotepadEntry extends Component { // eslint-disable-line no-unused-vars
  constructor(props) {
    ...
    this.breadCrumbs.push({
      title: 'Profile',
      sequence: 2,
      link: '/hadmin/retailer_management/view_branches'
    });
    this.breadCrumbs.push({
      title: props.params.Id,
      sequence: 3,
      link: '/hadmin/retailer_management/edit_branch/' + props.params.Id
    });
    this.breadCrumbs.push({
...
const mapStateToProps = (state) => {
  return {...state.retailer_notepad_data, ...state.page_data};
};
