...
const getTransactionCount = ( consumerId ) => {
  return (dispatch) => {
    ...
    const url = Endpoints.db + '/table/' + 'old_device' + '/count';
    const options = {
      ...genOptions,
...
const getTransactionData = (page, limit, consumerId ) => {
  return (dispatch) => {
    dispatch({ type: MAKE_REQUEST});
    //
    /* const payload = {'where': {'id': f}, 'columns': ['*']};*/
    ...
    const url = Endpoints.db + '/table/' + 'old_device' + '/select';
    const options = {
      ...genOptions,
      body: JSON.stringify(payload)
    };
...
const getAllTransactionData = (page, limit, consumerId ) => {
  const gotPage = page;
  const gotLimit = limit;
  /* Dispatching first one */
  return (dispatch) => {
    dispatch(getTransactionCount(consumerId))
      .then(() => {
        return dispatch(getTransactionData(gotPage, gotLimit, consumerId ));
      })
      .then(() => {
