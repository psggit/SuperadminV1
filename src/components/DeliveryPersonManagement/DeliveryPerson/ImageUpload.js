...
class ImageUpload extends Component {
  ...
  onUploadClick() {
    const fileComponent = document.getElementById(this.props.elementId);
            ...
            {/* http://130.211.246.199 */}
              {
                (imageUrl.length > 0) ?
                  ...
                  :
                  ( <input type="file" id={this.props.elementId} className="uploadImage"/> )
              }
            </div>
          </div>
          {
            (imageUrl.length === 0) ?
              (
                <div className={styles.upload_btn_layout}>
                  <button className={styles.upload_btn} onClick={ this.onUploadClick.bind(this) } >Upload Image</button>
                </div>
              ) : ''
          }
        </div>
